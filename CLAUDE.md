...
- `src/components/RequestPanel.tsx` — 请求面板，含 Body / Headers / Query / Cookies 四个 Tab；Body tab 含"提取提示词"按钮，识别 OpenAI / Anthropic 格式并输出结构化 Markdown
...
- `src/components/PreviewPanel.tsx` — Markdown 预览（react-markdown + remark-gfm + rehype-highlight），含目录（TOC）和复制按钮
